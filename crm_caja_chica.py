...
def generar_pdf_dashboard(conn):
    ...
    # Últimos movimientos
    pdf.set_font("Arial", "B", 14)
    pdf.cell(0, 10, "ÚLTIMOS MOVIMIENTOS", 0, 1, "L")
    ...
    pdf.cell(0, 10, "Reporte generado automáticamente - Sistema Caja Chica Enterprise", 0, 0, "C")
...
def vista_dashboard(conn):
    ...
    with col_pdf2:
        if st.button("📥 Descargar PDF", use_container_width=True):
            try:
                archivo_pdf = generar_pdf_dashboard(conn)
...
def vista_centros_costo(conn):
    ...
    with tab2:
        ...
        if centros:
            opciones_centros = {f"{c['codigo']} - {c['nombre']}": c for c in centros}
                    ...
                    if submitted:
                        try:
                            cursor.execute('''
                                ...
                                WHERE id=?
                            ''', (nuevo_codigo, nuevo_nombre, nueva_descripcion, centro_data['id']))
                            conn.commit()
    ...
    with tab3:
        ...
        if centros:
            opciones_centros = {f"{c['codigo']} - {c['nombre']}": c for c in centros}
            centro_eliminar = st.selectbox("Seleccionar Centro a Eliminar", list(opciones_centros.keys()))
            
            if centro_eliminar:
                centro_data = opciones_centros[centro_eliminar]
                st.warning(f"⚠️ ¿Está seguro de eliminar el centro: **{centro_data['nombre']}**?\n\nEsta acción no se puede deshacer.")
                
                if st.button("🗑️ Eliminar Centro", use_container_width=True, type="secondary"):
                    try:
                        ...
                        if count > 0:
                            st.error(f"⛔ No se puede eliminar: Tiene {count} movimientos asociados")
                        else:
...
def main():
    ...
    if page == 'dashboard':
        ...
        st.info("Vista de calendario en desarrollo...")
    elif page == 'config':
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("📊 Dashboard Ejecutivo PDF", use_container_width=True):
                archivo_pdf = generar_pdf_dashboard(conn)
                with open(archivo_pdf, "rb") as pdf_file:
                    PDFbyte = pdf_file.read()
                st.download_button(
        ...
        with col2:
            st.info("Más reportes próximamente...")
